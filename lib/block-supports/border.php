...
function gutenberg_apply_border_support( $block_type, $block_attributes ) {
	...
	if (
		...
		$preset_border_color          = array_key_exists( 'borderColor', $block_attributes ) ? "var:preset|border-color|{$block_attributes['borderColor']}" : null;
	...
	$styles       = $style_engine->generate( array( 'border' => $border_block_styles ) );
